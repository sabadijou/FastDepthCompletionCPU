...
def create_map(main_image,
               ...
               dilation_kernel_med=kernels.cross_kernel_5(),
               dilation_kernel_near=kernels.cross_kernel_7(),
               extrapolate=False,
               blur_type='bilateral',
               show_process=False):
    ...
    # Hole fill
    dilated = cv2.dilate(s4_blurred_depths, kernels.FULL_KERNEL_9)
    ...
    for i in range(6):
        ...
        dilated = cv2.dilate(s7_blurred_depths, kernels.FULL_KERNEL_5)
    ...
    image_jet = cv2.applyColorMap(
        np.uint8(depths_out / np.amax(depths_out) * 255),
        cv2.COLORMAP_JET)
    final_img = main_image + image_jet
    process_dict = None
    if show_process:
